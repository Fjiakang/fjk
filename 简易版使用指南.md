...
若设置自己的数据集，建议准备COCO格式的数据集。以SSDD数据集为例子，4个改动地方：    
    1. 新建 configs/_base_/datasets/ssdd_detection.py，copy coco_detection.py, 改动 data_root, dataset_type, img_norm_cfg, ann_file, img_prefix  
...
eg：
```
    2.1 画出某次运行时的classification loss：  
...
eg：
```
python tools/test.py configs/1/baseline/atss_v6_myfpnv12_1_vhrvoc_topk10.py work_dirs/atss_v6_myfpnv12_1_vhrvoc_topk10/latest.pth --out last.pkl
```
```
...
运行命令：  
```
python demo/heatmap_visual.py {figure} {config} {weight}  
```
eg：  
```
python demo/heatmap_visual.py data/VHR_voc/JPEGImages/121.jpg work_dirs/atss_v6_myfpnv12_1_vhrvoc_topk10/atss_v6_myfpnv12_1_vhrvoc_topk10.py work_dirs/atss_v6_myfpnv12_1_vhrvoc_topk10/latest.pth
```
#### 方式二
##### heatmap
利用demo/heatmap_visual.py中的draw_single_map函数. 生成FPN（neck）或backbone的heatmap：    
具体步骤如下：  
1.将demo/heatmap_visual.py进行注释（否则会出现ImportError: cannot import name 'inference_detector' from 'mmdet.apis'）  
2.在其neck某一层后调用draw_single_map函数即可.  
eg：
```
    对于configs/2/ours_2_3_verify_3/vhr/atss_fnpndrsncv3_555_atssassign_vhr_cosv3giou_2.py，在其neck某一层输出heatmap，
    在mmdet/models/necks/fpn_drsncv3.py中的某一层添加以下代码：
...
生成FPN+Backone的Class Activation Map，步骤如下：  
1.对以下部分修改（对部分代码注释，解注释即可）：  
 /home/aspeny/zbj/mmdet/apis/inference.py , 1处  
 mmdet/models/detectors/single_stage.py ， 5处  
2.运行demo/heatmap_visual.py（即利用demo/heatmap_visual.py的draw_feature_map函数）
```
python demo/heatmap_visual.py {img} {config} {weight.pth} [--save_dir file]
```
eg：
```
对于configs/2/ours_2_3_verify_3/vhr/atss_fnpndrsncv3_555_atssassign_vhr_cosv3giou_2.py,运行以下命令：

python demo/heatmap_visual.py demo/demo.jpg configs/2/ours_2_3_verify_3/vhr/atss_fnpndrsncv3_555_atssassign_vhr_cosv3giou_2.py work_dirs/atss_fnpndrsncv3_555_atssassign_vhr_cosv3giou_2/latest.pth --save_dir result
```
若要对文件夹下多张图片进行Class Activation Map处理，可以运行heatmap.sh
```
sh heatmap.sh
```