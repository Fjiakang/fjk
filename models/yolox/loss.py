...
from libs.utils.boxes_utils import bboxes_iou
...
class get_loss(nn.Module):
    ...
    def __call__(self, x_shifts, y_shifts, expanded_strides, outputs, origin_preds, target):
        ...
        # label_cut = labels
        nlabel = (target.sum(dim=2) > 0).sum(dim=1)  # number of objects
        total_num_anchors = outputs.shape[1]
        # for batch_idx in range(len(target)):
        #     total_num_anchors += target[batch_idx].shape[0]
        # total_num_anchors = outputs.shape[1]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            #num_gt = target[batch_idx].shape[0]
            if num_gt == 0:
                ...
                # gt_bboxes_per_image = target[batch_idx][:, :4]
                # gt_classes = target[batch_idx][:, 4]

                # bboxes_preds_per_image = bbox_preds[batch_idx]
                gt_bboxes_per_image = target[batch_idx, :num_gt, 1:5]
                gt_classes = target[batch_idx, :num_gt, 0]
                bboxes_preds_per_image = bbox_preds[batch_idx]
                ...
                cls_target = F.one_hot(gt_matched_classes.to(torch.int64), self.args.num_classes) * pred_ious_this_matching.unsqueeze(-1)
                ...
                if self.use_l1:
                    l1_target = self.get_l1_target(
                        ...
                        expanded_strides[0][fg_mask].cuda(),
                        x_shifts=x_shifts[0][fg_mask].cuda(),
                        y_shifts=y_shifts[0][fg_mask].cuda(),
                    )
        ...
        return {"total_loss": loss, "iou_loss": reg_weight * loss_iou, "conf_loss": loss_obj, "cls_loss": loss_cls, "l1_loss": loss_l1}, num_fg / max(num_gts, 1)
    ...
    def get_assignments(
        ...
        gt_cls_per_image = F.one_hot(gt_classes.to(torch.int64), self.args.num_classes).float().unsqueeze(1).repeat(1, num_in_boxes_anchor, 1)
    ...
    def get_in_boxes_info(
        ...
        expanded_strides_per_image = expanded_strides_per_image.to(self.device)
