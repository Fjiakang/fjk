...
from mmdet.datasets.aitod import AITODDataset
#from datasets.aitod import AITODDataset
...
#            batch_classes=batch_classes.cuda()
            ...
            cls_loss, loc_loss, cls_neg_loss = self.criterion(out, targets)
            #loss=losses[-1]
            #loss_mean += total_loss
            total_loss = cls_loss+loc_loss+cls_neg_loss
            total_loss.backward()
            self.optimizer.step()

            #end_time=time.time()
           # cost_time=int((end_time-start_time)*1000)
            self.save_loggin_print(epo, { "cls_loss": cls_loss, "loc_loss": loc_loss, "cls_neg_loss": cls_neg_loss}, iteration, num_iters)
    ...
    def inference(self, testset,  fold=None, from_train=-1):
        ...
        #all_boxes = [[[] for _ in range(self.num_classes)] for _ in range(len(testset))]
        ...
        if self.args.det_dataset == 'VOC':
            ...
            AITODDataset.evaluate(testset,all_boxes)
...
class DetectHead(nn.Module):     
    def __init__(self):
        ...
        self.max_detection_boxes_num=3000
    ...
    def forward(self,inputs):
        ...
        cnt_logits,_=self._reshape_cat_out(inputs[2],self.strides)#[batch_size,sum(_h*_w),1]
