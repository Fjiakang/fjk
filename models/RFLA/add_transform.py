...
def rfla_collate_fn(data):
        ...
        for i in range(batch_size):
            ...
            pad_boxes_list.append(boxes_list[i].float())
            pad_classes_list.append(classes_list[i].float()-1)
        

        #batch_boxes=pad_boxes_list
        #batch_classes=torch.stack(pad_classes_list)
        batch_imgs=torch.stack(pad_imgs_list)
     
        return batch_imgs,pad_boxes_list,pad_classes_list
...
class Test_transform():
    ...
    def __call__(self, img, bboxs, label,index):
        ...
        batch_imgs,batch_boxes,batch_classes = rfla_collate_fn(data)
